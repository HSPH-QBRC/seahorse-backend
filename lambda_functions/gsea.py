...
def lambda_handler(event, context):
    ...
    sql = "SELECT pvalue, pathway, ranks FROM gsea WHERE varname = %s AND tissue = %s AND pvalue is not null ORDER BY pvalue LIMIT %s OFFSET %s"

    sql_count = "SELECT COUNT(*) FROM gsea WHERE varname = %s AND tissue = %s AND pvalue is not null"
    
    with connection:
        with connection.cursor() as cursor:
            cursor.execute(sql, (params["meta"], params["tissue"], params["limit"], params["offset"]))
            limit = int(params["limit"])
            result = cursor.fetchmany(limit)
            
            cursor.execute(sql_count, (params["meta"], params["tissue"]))
